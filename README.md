...
A deal showing an $8M sale at month 36 looks entirely different when the outstanding mortgage balance is subtracted in the same cash flow period.

On a $5M acquisition at 65% LTV with 60-month French amortization at 4.5%, the outstanding balance at month 36 is approximately $2.3M.

The net cash flow to equity at exit is not $8M. It is $8M minus $2.3M minus that month's scheduled interest and principal payment.
...
Analysts who net VAT in the period it is paid, or who omit the lag entirely, misstate peak equity requirement by the full VAT balance outstanding during the lag window.
...
$400,000 of unmodeled working capital requirement is the difference between a deal that fits within an equity budget and one that requires an emergency revolving draw.
...
When a downside capex multiplier of 1.20× is active, the correct method runs the full 121-period cash flow with the adjusted input and derives IRR from that new series.
...
Higher capex increases equity invested, lengthens the construction drawdown period, increases VAT float, and reduces the exit equity multiple simultaneously.  
...
Refund received after configurable lag (0–12 months), implemented via INDEX offset into the historical VAT paid array.  
...
Exit proceeds fire at `TARGET_EXIT_MONTH`, calculated as direct sale price or `NOI / EXIT_CAP_RATE` for Buy-to-Rent.
...
5×5 live recalculation. Exit value and capex budget each varied across five multipliers (−30% to +30%).  
...
**Deal profile:** Full Development. $5M acquisition at 4% transfer tax. $2M construction over 24 months, S-curve distribution. 65% LTV, French amortization, 4.5% annual interest, 60-month tenor. Sale at month 36 for $8M. 20% VAT on capex, 6-month refund lag.
...
It is worse by a larger margin because equity invested increases, VAT float increases, and exit proceeds decrease simultaneously.  
The compounding effect is only visible through full cash flow recalculation — not through rate arithmetic.

---

## Why Parametric Models Work This Way

A financial model that requires manual cell edits for every scenario test is a calculator with extra steps.

Parametric design separates three concerns:

**Structure** — the formula chain that converts inputs into outputs. Fixed.  
Modifying `CAPEX_TOTAL_BUDGET` does not change any formula; it changes the input a formula reads.
...
Changing `ACTIVE_SCENARIO` from 1 to 3 simultaneously adjusts capex, revenue, cap rate, and rental income via a single `INDEX` on the scenario matrix.  
No cell references change. No formulas break.

The practical consequence: any analyst can stress-test the model without understanding its internal structure. Any reviewer can audit the structure without running scenarios.  
...
The complete Excel workbook contains all 32 named ranges, full formula architecture across 4 sheets, and 121-month per-column formula chains for every calculation row.
...
The logic is correct. The formulas work. But the model cannot be reviewed without the file, cannot be demonstrated without screen-sharing, and cannot be stress-tested by anyone who did not build it.
